// Point d'entrée : la ville en 3D, la voiture, les portails, et le passage
...
import { construireVille, DISTRICTS } from './ville.js';
import { PLACE, RAYON_VILLE } from './plan.js';
import { construireCiel } from './decor.js';
import { creerVoiture } from './voiture.js';
...
if (!SANS.has('fog')) scene.fog = new THREE.FogExp2(0x4a3652, 0.0058);
...
const voiture = creerVoiture(scene, camera, entrees, ville.occupe);
voiture.teleporter(0, 21, 0);
if (CAPTURE && CAPTURE.length >= 3) voiture.teleporter(CAPTURE[1], CAPTURE[2], CAPTURE[3] || 0);
...
function verifierPortails() {
  ...
  for (const p of ville.portails) {
    const dx = voiture.pos.x - p.x, dz = voiture.pos.z - p.z;
  ...
  // le quartier courant : la place au centre, sinon le portail le plus proche
  const r = Math.hypot(voiture.pos.x, voiture.pos.z);
  if (r < PLACE.rayon + 6) { quartierEl.textContent = 'LA PLACE · point de départ'; return; }
  let q = null, qd = 1e9;
  for (const d of DISTRICTS) { const dd = Math.hypot(voiture.pos.x - d.x, voiture.pos.z - d.z); if (dd < qd) { qd = dd; q = d; } }
...
function terminerJeu(j, p, gagne, res) {
  ...
  ouvrirPanneau(
    `<p class="pan-k" style="color:${acc}">${tout ? 'Ville traversée' : 'Clé acquise'}</p>
     ...
     ${tout ? `<p class="pan-w">Vous avez traversé une école, cinq postes et un scénario. La machine peut écrire le
       ...
       <p class="pan-w"><b>${PROFIL.nom}</b> · <a href="mailto:${PROFIL.email}">${PROFIL.email}</a></p>`
            : `<p class="pan-w">${etat.cles.length} quartier${etat.cles.length > 1 ? 's' : ''} sur ${total}. La boussole indique le suivant.</p>`}`,
    [{ label: 'Retour à la ville', fort: true, action: fermerPanneau }]);
}
...
function dessinerPoi() {
  ...
  const visibles = ville.portails.map((p, i) => {
    const dist = Math.hypot(voiture.pos.x - p.x, voiture.pos.z - p.z);
    v3.set(p.x, 12.5, p.z).project(camera);
    const devant = v3.z < 1 && Math.abs(v3.x) < 1.1 && Math.abs(v3.y) < 1.1;
    return { p, i, dist, sx: (v3.x + 1) / 2 * w, sy: (1 - (v3.y + 1) / 2) * h, ok: devant && dist > 5 && dist < 150 };
  ...
  for (const v of visibles) {
    ...
    e.style.top = sy + 'px';
    e.style.opacity = v.dist < 20 ? 1 : Math.max(0.45, 1 - (v.dist - 20) / 170);
...
// boussole : le prochain quartier du parcours, sa distance, et où il est par rapport au cap
const capEl = el('cap'), capFleche = capEl.querySelector('i'), capTexte = capEl.querySelector('span');
function dessinerCap() {
  const p = ville.portails.find(p => !etat.cles.includes(p.id));
  if (!p) { capEl.hidden = true; return; }
  const dx = p.x - voiture.pos.x, dz = p.z - voiture.pos.z;
  const dist = Math.hypot(dx, dz);
  // angle entre le cap de la voiture et la direction du portail, 0 = droit devant
  const versPortail = Math.atan2(-dx, -dz);
  let rel = versPortail - voiture.yaw;
...
// ---------------------------------------------------------------- minicarte
// Le plan de la ville vu du ciel : les rues, la place, les portails, la voiture.
function dessinerMinicarte() {
  const w = minicarte.width, h = minicarte.height;
  mctx.clearRect(0, 0, w, h);
  mctx.fillStyle = 'rgba(11,14,17,.82)';
  mctx.fillRect(0, 0, w, h);
  const E = Math.min(w, h) / (RAYON_VILLE * 2 + 8);
  const X = x => w / 2 + x * E, Z = z => h / 2 + z * E;
  mctx.strokeStyle = '#223038';
  for (const s of ville.routes) {
    mctx.lineWidth = Math.max(1, s.w * E * 0.6);
    mctx.beginPath(); mctx.moveTo(X(s.x1), Z(s.z1)); mctx.lineTo(X(s.x2), Z(s.z2)); mctx.stroke();
  }
  mctx.fillStyle = '#1d2a24';
  mctx.beginPath(); mctx.arc(X(0), Z(0), PLACE.rayon * E, 0, Math.PI * 2); mctx.fill();
  for (const p of ville.portails) {
    mctx.fillStyle = etat.cles.includes(p.id) ? p.accent.hex : 'rgba(0,0,0,0)';
    mctx.strokeStyle = p.accent.hex; mctx.lineWidth = 1.5;
    mctx.beginPath(); mctx.arc(X(p.x), Z(p.z), 3.5, 0, Math.PI * 2); mctx.fill(); mctx.stroke();
  }
  mctx.save();
  mctx.translate(X(voiture.pos.x), Z(voiture.pos.z));
  mctx.rotate(-voiture.yaw);
...
if (CAPTURE) {
  voiture.mettreAJour(1 / 60); ville.animer(0); verifierPortails();
  renderer.render(scene, camera); dessinerMinicarte(); dessinerPoi(); dessinerCap();
  window.__capturePret = true;
  setTimeout(() => { voiture.mettreAJour(1 / 60); renderer.render(scene, camera); }, 400);
  ...
  try {
    ...
    window.__journal && window.__journal(`DIAG canvas ${renderer.domElement.width}x${renderer.domElement.height} conteneur ${conteneur.clientWidth}x${conteneur.clientHeight} appels ${renderer.info.render.calls} tri ${renderer.info.render.triangles} pixel ${[...px].join(',')} cam ${camera.position.toArray().map(v => v.toFixed(1)).join(',')} voiture ${voiture.pos.toArray().map(v => v.toFixed(1)).join(',')} batiments ${ville.batiments.length} routes ${ville.routes.length}`);
...
// on part de la place : on l'indique une fois, au départ
setTimeout(() => {
  if (etat.cles.length) return;
  invite.innerHTML = `Flèches pour conduire · <kbd>Maj</kbd> accélère · sept quartiers autour de la place, la boussole indique le premier`;
  invite.hidden = false;
  setTimeout(() => { if (!portailProche) invite.hidden = true; }, 7000);
}, 800);

window.__ville = { etat, voiture, sonde: voiture, ville, DISTRICTS, JEUX, lancerJeu, entrerPortail };
